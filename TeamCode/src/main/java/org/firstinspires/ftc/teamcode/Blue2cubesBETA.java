...
@Autonomous(name="BlueLeft2CubesBETA", group="Robot")

public class Blue2cubesBETA extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        driveBackwards(calculateTicks(-12.5),1);
        rotate(-110,0.5);
        driveLeft(calculateTicks(55),1.0,1);
        driveForward(calculateTicks(12.5), 1);
        ClawServo.setPosition(CLOSE);
        driveBackwards(calculateTicks(-12.5),1);
        rotate(-110,0.5);
        driveLeft(calculateTicks(62),1.0,1);
        moveClaw(1,765);
        driveForward(calculateTicks(12.5), 1);
        moveClaw(-1,765);
        ClawServo.setPosition(OPEN);
        driveBackwards(calculateTicks(-25),1);
        driveRight(calculateTicks(62),1.0,1);

    }
    //will calculate ticks for one foot
    //for the robot to move 1 foot call calculateTicks 10 inches
    public static double calculateTicks(double distanceToMoveInInches) {
        ...
        double ticksNeeded = (rotationsNeeded * ticksPerRotation);
    ...
    public void driveRight(double targetTicks, double power, long sleep){
        ...
        while (!rightStop && !leftStop) {
            ...
            telemetry.addData("Left tick", getLeftTicks());
            telemetry.addData("Target tick", targetTicks);
            telemetry.addData("Center Ticks", getCenterTicks());
            telemetry.update();
    ...
    public void rotate(double targetDegrees, double power){
        ...
        double deltaticks;//need to know how much we have moved
        double remainingTicks;// Calculate remaining distance
        double test = 14.4 * Math.PI;
        double targetTicks = calculateTicks(test);

        Boolean stop = false;
        while(!stop){
            deltaticks = Math.abs (getRightTicks() + getLeftTicks());
            remainingTicks = targetTicks - deltaticks;
            double percentComplete = (-remainingTicks + targetTicks) / targetTicks;
            ...
            telemetry.addData("complete", percentComplete);
            telemetry.addData("target", targetTicks);
            telemetry.addData("delta", deltaticks);
            telemetry.addData("Left tick", getLeftTicks());
            telemetry.addData("right tick", getRightTicks());
            telemetry.update();
